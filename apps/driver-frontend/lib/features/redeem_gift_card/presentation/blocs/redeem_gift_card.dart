...
class RedeemGiftCardBloc extends Cubit<RedeemGiftCardState> {
  ...
  void onCodeChanged(String code) {
    emit(state.copyWith(formState: const AppFormState.idle(), code: code));
  }

  void onSubmitted() async {
    if (state.code == null) throw Exception('Code is null');

    emit(state.copyWith(formState: const AppFormState.busy()));

    final result = await _repository.redeemGiftCard(code: state.code!);
    emit(
      state.copyWith(
