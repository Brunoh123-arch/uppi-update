...
class DesktopLayoutDelegate extends MultiChildLayoutDelegate {
  ...
  void performLayout(Size size) {
    ...
    positionChild(
      bottomSheetLayoutId,
      Offset(0, size.height - bottomSheetSize.height),
    );
    ...
    if (bottomSheetSize.height < 50) {
      layoutChild(
        mapLayoutId,
        BoxConstraints(maxWidth: size.width - 400, maxHeight: size.height),
      );
      positionChild(mapLayoutId, Offset.zero);
      layoutChild(navbarId, const BoxConstraints(maxWidth: 400));
      positionChild(navbarId, Offset(size.width - 400, 80));
      ...
      positionChild(navbarId, const Offset(0, 80));
