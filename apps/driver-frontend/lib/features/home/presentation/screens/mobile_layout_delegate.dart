...
class MobileLayoutDelegate extends MultiChildLayoutDelegate {
  ...
  MobileLayoutDelegate({required this.isMapFull});
  ...
  void performLayout(Size size) {
    ...
    final myLocationButtonSize = layoutChild(
      myLocationButtonId,
      const BoxConstraints(),
    );
    final navigationButtonSize = layoutChild(
      navigateButtonId,
      const BoxConstraints(),
    );
    positionChild(
    ...
    if (cardSize.height < size.height) {
      positionChild(cardLayoutId, Offset(0, size.height - cardSize.height));
      layoutChild(
        mapLayoutId,
        BoxConstraints(
          maxWidth: size.width,
      ...
      final navBarSize = layoutChild(
        navbarId,
        BoxConstraints(maxWidth: size.width - 32),
      );
      positionChild(navbarId, const Offset(16, 16));
      positionChild(
        searchRadiusButtonId,
        Offset(
      ...
      layoutChild(navbarId, const BoxConstraints(maxWidth: 0, maxHeight: 0));
      positionChild(navbarId, const Offset(16, 16));
      positionChild(searchRadiusButtonId, Offset(0, -searchRadiusButtonSize.height - 100));
      // Mapa sempre visível no fundo (tela cheia), mesmo quando o card cobre tudo
      layoutChild(
        mapLayoutId,
        BoxConstraints(
          maxWidth: size.width,
