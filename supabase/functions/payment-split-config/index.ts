...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: settings } = await supa
      .from('app_settings')
      .select('key, value')
      .in('key', ['platform_fee_percent', 'driver_share_percent', 'min_platform_fee', 'flat_fee_amount']);

    const config: Record<string, string> = {};
    settings?.forEach((s: any) => { config[s.key] = s.value; });

    const platformFeePercent = Number(config['platform_fee_percent']) || 0;
    const driverSharePercent = Number(config['driver_share_percent']) || 100;
    const minPlatformFee = Number(config['min_platform_fee']) || 1.5;
    const flatFeeAmount = Number(config['flat_fee_amount']) || 1.5;
